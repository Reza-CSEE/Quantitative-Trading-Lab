...
class BreakoutStrategy(bt.Strategy):
    ...
    def next(self):
        # بررسی آیا موقعیت باز داریم یا نه
        if not self.position:
            if self.data.close[0] > self.highest_high[-1]:  # Breakout above resistance
                cash_available = self.broker.get_cash()  # دریافت میزان سرمایه نقدی
                share_size = cash_available * 0.10 / self.data.close[0]  # خرید ۱۰٪ از سرمایه نقدی
                self.buy(size=round(share_size))  # گرد کردن تعداد سهام به عدد صحیح
        else:
            if self.data.close[0] < self.lowest_low[-1]:  # Drop below support
                self.sell(size=self.position.size)  # فروش کل مقدار خریداری شده
